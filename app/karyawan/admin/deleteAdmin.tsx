...
const DeleteAdmin = (myProp: props) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            const url = `/employee/${myProp.admin.id}` // harus sesuai dengan yg di postman
            ...
            if (response.data.success == true) {
                toast(message,
                    {
                        containerId: `toastDelete-${myProp.admin.id}`,
                        type: "success"
                    }
                )
                setShow(false)
    ...
    return (
        <div>
            <ToastContainer containerId={`toastDelete-${myProp.admin.id}`} />
            <button type="button"
                onClick={() => openModal()}
