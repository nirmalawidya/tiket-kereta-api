...
const DeletePelanggan = (myProp: props) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            const url = `/customer/${myProp.pelanggan.id}` // harus sesuai dengan yg di postman
            ...
            if (response.data.success == true) {
                toast(message,
                    {
                        containerId: `toastDelete-${myProp.pelanggan.id}`,
                        type: "success"
                    }
                ) 
                setShow(false)
    ...
    return (
        <div>
            <ToastContainer containerId={`toastDelete-${myProp.pelanggan.id}`} />
            <button type="button"
                onClick={() => openModal()}
